...
{
    public class GetBooksByAuthorCommand : ICommand<List<BookClient>>
    {
        public List<BookClient> Execute(object parameter)
        {
            var authorName = parameter as string;
            var libraryWebServiceSoapClient = WebService.GetService();
            var books = libraryWebServiceSoapClient.GetBooksByAuthor(authorName);
